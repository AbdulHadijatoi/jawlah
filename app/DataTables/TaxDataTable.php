...
class TaxDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->editColumn('title', function($tax){
                return '<a class="btn-link btn-link-hover" href='.route('tax.create', ['id' => $tax->id]).'>'.$tax->title.'</a>';
            })
            ->editColumn('status' , function ($tax){
                return '<div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                    <div class="custom-switch-inner">
                        <input type="checkbox" class="custom-control-input bg-primary change_status" data-type="tax_status" '.($tax->status ? "checked" : "").'   value="'.$tax->id.'" id="'.$tax->id.'" data-id="'.$tax->id.'">
