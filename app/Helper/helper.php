...
function removeArrayValue(array $array,$find)
{
...
function getServiceTimeSlot($provider_id)
{
    ...
    foreach ($days as $value) {
        ...
        $slot = $slot->pluck('start_at')->toArray();

        $bookings = \App\Models\Booking::where('provider_id', $provider_id)
            ->where('booking_day', $value)
            ->whereIn('booking_slot', $slot)
            ->groupBy('booking_slot')
            ->havingRaw('COUNT(*) < ?', [$handyman_count])
            ->pluck('booking_slot')
            ->toArray();
...
function set_admin_approved_cash($payment_id){
    ...
    ->where('action','handyman_send_provider')->where('status','pending_by_admin')->first();
